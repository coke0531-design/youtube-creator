#!/usr/bin/env python3
# Vendored from geeklee/srt-whiteboard-animation (MIT, commit 696a724) — LICENSE.srt-whiteboard-animation
# youtube-creator 개작(2026-08-16): 절차적 펜촉 색을 앰버(#F59E0B)로.
"""
流式笔迹动画 - 单图渲染入口
...
def _procedural_tip(target_h: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    # 圆头笔尖（暖色，模拟油墨）
    tip_cy = int(h * 0.70)
    cv2.circle(rgba, (w // 2, tip_cy), max(3, w // 4), (11, 158, 245), thickness=-1)  # 앰버 #F59E0B (BGR)
